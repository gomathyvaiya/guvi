...
if(isset($_POST['myData'])) {
	...
	$lang1=$result->lang1;
	$skill=$result->skill;
	$mentor_hide=$result->mentor_hide;
	$ip_add=$_SERVER['REMOTE_ADDR'];
	$result= array();
	$hashval=hash(PBKDF2_HASH_ALGORITHM, $user_name);
 $arr = array('email' => $user_name, 'password' => $password, 'name' => $name, 'language' => $lang1, 'skill' => $skill, 'usertype' => 0, 'hashval' => $hashval);
		  ...
		  if($stmt = $mysqli -> prepare($sql)) 
		  {
			...
			{		
				...
				if($stmt = $mysqli -> prepare($sql1)) {
							  $stmt -> bind_param('ssssssssss',$user_name,$password,$mydatajson,$name,$hashval, $lang1,$skill,$mentor_hide,$timestamp,$timestamp);
							  $stmt -> execute();
							  $stmt -> close();
